  ...
  const assetProfitLabel = props.ownerships.map((value, index) => (
    <View
      ...
      <P1Text
        ...
        label={`$ ${parseFloat(value[1]).toFixed(2)}`} />
    </View>
  ));
  ...
  return (
    <View
      ...
      <View
        ...
        <P1Text style={{ marginBottom: 20 }} label={i18n.t('dashboard_label.asset_value_graph')} />
        <View
          style={{
            flexDirection: 'row',
      ...
      {!totalValue ?
        <>
          ...
          <P1Text style={{ marginTop: 25, textAlign: 'center' }} label={i18n.t('dashboard.no_ownership')} />
        </> : (
          <>
            <PieChart
              style={{ height: 200 }}
