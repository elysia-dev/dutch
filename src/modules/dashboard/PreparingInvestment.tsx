    ...
    <View style={{ backgroundColor: "#fff", width: '100%', height: "100%", padding: 20 }}>
      ...
      <View style={{ marginTop: 30, width: "100%" }}>
        <StatusButton
          ...
          color={(kycStatus === KycStatus.PENDING || kycStatus === KycStatus.SUCCESS)}
          onPress={() => {
        ...
        />
        <StatusButton
          title={ethAddresses?.length > 0 ? i18n.t('dashboard.wallet_connected') : i18n.t('dashboard.no_wallet')}
          color={ethAddresses?.length > 0}
          onPress={() => {
            if (!(ethAddresses?.length > 0)) {
              navigation.navigate("More", { screen: MorePage.RegisterEthAddress });
            }
          }}
        />
      </View>
