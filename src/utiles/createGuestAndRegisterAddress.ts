...
const createGuestAndRegisterAddress = async (
  language: string,
): Promise<ITokenAndRequestId> => {
  try {
    const guestRes: AxiosResponse<AccountResponse> = await espressoClient.post(
      '/users/guest',
      { language },
    );
    ...
    const ethAddressRes: AxiosResponse<TransactionRequestResponse> =
      await authEspressoClient.post('/ethAddress');
    return {
